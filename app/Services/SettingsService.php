...
class SettingsService
{
    ...
    {
        return [
            'CAF' => ['ISEE', '730', 'IMU', 'Bonus Casa'],
            'PATRONATO' => ['NASpI', 'Invalidità civile', 'Pensione di reversibilità', 'Disoccupazione agricola'],
        ];
    }
    ...
    /**
     * @return array{success:bool,errors:array<int,string>,services:array<string,array<int,string>>,auto_generated:bool}
     */
    ...
    public function saveCafPatronatoServices(array $services, int $userId, bool $autoGenerated = false): array
    {
        ...
        foreach ($input as $typeKey => $rows) {
            ...
            foreach ($rows as $index => $row) {
                ...
                $seen[$hash] = true;
                $normalized[$typeKey][] = mb_substr($value, 0, 120);
                $totalServices++;
            }
        }
    ...
    private function normalizeCafPatronatoServices($services, ?array $allowedKeys = null): array
    {
        if (!is_array($services)) {
            return self::defaultCafPatronatoServices();
        }
        ...
        foreach ($input as $typeKey => $rows) {
            ...
            foreach ($rows as $row) {
                ...
                $seen[$hash] = true;
                $normalized[$typeKey][] = mb_substr($trimmed, 0, 120);
            }
        }
