        ...
        'windtre_consumer' => [
            ...
            'actions' => [
                ...
                [
                    ...
                    'script' => '(function(){var selectors="a[href*=\'check-coverage-super-fibra\' i], .js-external-popup-open, [data-button-text-desk*=\'copertura fibra\' i]";var fallback="https://www.windtre.it/content/experience-fragments/windtre/it/it/site/modale-verifica-copertura/check-coverage-super-fibra.html";var openModal=function(){var btn=document.querySelector(selectors);if(!btn){return false;}try{btn.scrollIntoView({behavior:\'smooth\',block:\'center\'});}catch(e){}btn.click();return true;};if(window.__windtreModalInterval){clearInterval(window.__windtreModalInterval);}if(openModal()){return;}var attempts=0;window.__windtreModalInterval=setInterval(function(){attempts++;if(openModal()){clearInterval(window.__windtreModalInterval);}else if(attempts>=60){clearInterval(window.__windtreModalInterval);window.location.href=fallback;}},500);})();',
                    'continue_on_error' => true,
                ],
                ['action' => 'wait_for', 'selector' => '#js-external-popup-placeholder iframe, #js-external-popup-placeholder form, form[action*="check-coverage-super-fibra" i], form input:is([name*="address" i],[placeholder*="indirizzo" i])', 'timeout' => 40],
                ['action' => 'pause', 'milliseconds' => 800],
                ['action' => 'switch_to_frame', 'selector' => '#js-external-popup-placeholder iframe', 'continue_on_error' => true],
                ['action' => 'wait_for', 'selector' => 'form input:is([name*="address" i],[placeholder*="indirizzo" i])', 'timeout' => 30],
