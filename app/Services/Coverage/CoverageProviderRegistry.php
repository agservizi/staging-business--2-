...
final class CoverageProviderRegistry
{
    ...
    private const PROVIDERS = [
        ...
        'windtre_consumer' => [
            ...
            'actions' => [
                ['action' => 'wait_for', 'selector' => 'body', 'timeout' => 20],
                [
                    'action' => 'execute_script',
                    'script' => '(function(){if(window.__windtreCoverageWatcher){return;}window.__windtreCoverageWatcher=true;var selector="#copertura .coverage-fields, #copertura .coverage, .coverage-fields";var scrollToCoverage=function(){var target=document.querySelector(selector);if(target){try{target.scrollIntoView({behavior:\'smooth\',block:\'start\'});}catch(e){window.scrollTo(0,target.getBoundingClientRect().top+window.scrollY-120);}return true;}window.scrollTo(0,document.body.scrollHeight);return false;};if(window.__windtreCoverageInterval){clearInterval(window.__windtreCoverageInterval);}if(scrollToCoverage()){return;}var attempts=0;window.__windtreCoverageInterval=setInterval(function(){attempts++;if(scrollToCoverage()||attempts>=30){clearInterval(window.__windtreCoverageInterval);}},700);})();',
                    'continue_on_error' => true,
                ],
                ['action' => 'pause', 'milliseconds' => 400],
                ...
                ['action' => 'wait_for', 'selector' => '.cmp-cookie-consent__accept button, button[id*="accept" i], button[class*="accept" i], button[name*="accept" i], button[data-cookie*="accept" i], button[data-action*="accept" i], button[data-gtm*="consent" i], button[data-testid*="accept" i], #cookiebar-accept, #cookiebar button.accept, #onetrust-accept-btn-handler, #cookieModal, .cookie-modal, #cookieBanner, .cookie-banner, .cb-header, .cb-container', 'timeout' => 12, 'continue_on_error' => true],
                ...
                [
                    'action' => 'execute_script',
                    'script' => '(function(){var modal=document.querySelector("#cookieModal, .cookie-modal, #cookieBanner, .cookie-banner, .cb-header, .cb-container");if(!modal){return;}var buttons=modal.querySelectorAll("button, a");var target=null;buttons.forEach(function(btn){var text=(btn.innerText||btn.textContent||"").toLowerCase();if(!target&&(text.indexOf("continua")!==-1||text.indexOf("senza")!==-1||text.indexOf("accept")!==-1||text.indexOf("accetta")!==-1)){target=btn;}});if(!target){target=buttons[0];}if(target){try{target.click();}catch(e){}}})();',
                    'continue_on_error' => true,
                ],
                ['action' => 'execute_script', 'script' => '(function(){var overlays=document.querySelectorAll("#cookiebar,.cmp-cookie-consent,#onetrust-consent-sdk,#cookieModal,.cookie-modal,#cookieBanner,.cookie-banner,.cb-header,.cb-container");overlays.forEach(function(el){el.style.display="none";el.style.pointerEvents="none";});})();', 'continue_on_error' => true],
                ...
                ['action' => 'wait_for', 'selector' => '#js-external-popup-placeholder iframe, #js-external-popup-placeholder form, form[action*="check-coverage-super-fibra" i], form input:is([name*="address" i],[placeholder*="indirizzo" i]), .coverage-fields input:is(#pac-input,[placeholder*="indirizzo" i])', 'timeout' => 55],
