        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'url' => clienti_module_url('view', ['id' => (int) $row['id']]),
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => caf_patronato_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => energia_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
                    'status' => $row['stato'] ?? null,
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => entrate_uscite_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
                    'status' => $row['stato'] ?? null,
                    'icon' => 'fa-receipt',
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => documenti_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
                    'status' => $row['stato'] ?? null,
                    'icon' => 'fa-file-lines',
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $displayName,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => impostazioni_module_url('users') . '#user-row-' . (int) $row['id'],
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => ticket_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $items[] = [
                    ...
                    'url' => impostazioni_module_url('notifications'),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['created_at'] ?? ''),
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => email_marketing_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
                    'status' => $row['status'] ?? null,
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => email_marketing_module_url('subscribers') . '#subscriber-row-' . (int) $row['id'],
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['created_at'] ?? ''),
                    'status' => $row['status'] ?? null,
                    'icon' => 'fa-user-check',
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => opportunities_module_url('detail', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => appuntamenti_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => aci_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
                    'status' => $row['stato'] ?? null,
                    'icon' => 'fa-car',
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => anpr_module_url('view_request', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
                    'status' => $row['stato'] ?? null,
                    'icon' => 'fa-id-card',
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => cie_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
                    'status' => $row['stato'] ?? null,
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => digitali_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
                    'status' => $row['stato'] ?? null,
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => fedelta_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => curriculum_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
                    'status' => $row['status'] ?? null,
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => $row['cliente_id'] ? clienti_module_url('view', ['id' => (int) $row['cliente_id']]) : clienti_module_url('index'),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
                    'status' => $row['stato'] ?? null,
                    'icon' => 'fa-truck',
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => brt_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
                    'status' => $row['status'] ?? null,
                    'icon' => 'fa-truck-fast',
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => brt_module_url('manifest', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => posta_telematica_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['created_at'] ?? ''),
                    'status' => $row['status'] ?? null,
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => telegrammi_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['created_at'] ?? ''),
                    'status' => $row['stato'] ?? null,
                    'icon' => 'fa-paper-plane',
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => visure_cr_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['updated_at'] ?? ''),
                    'status' => $row['stato'] ?? null,
                    'icon' => 'fa-building',
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => logistici_module_url('view', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['created_at'] ?? ''),
                    'status' => $row['status'] ?? null,
                    'icon' => 'fa-box',
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => '',
                    'url' => logistici_module_url('report', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['created_at'] ?? ''),
                    'status' => $row['status'] ?? null,
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => iliad_module_url('generate_pdf', ['id' => (int) $row['id']]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['created_at'] ?? ''),
                    'status' => null,
                    'icon' => 'fa-sim-card',
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $items[] = [
                    ...
                    'title' => $title,
                    'subtitle' => implode(' • ', $subtitleParts),
                    'url' => report_module_url('download_daily_report', ['date' => (string) ($row['report_date'] ?? '')]),
                    'relevance_score' => (int) ($row['relevance_score'] ?? 1),
                    'date' => (string) ($row['created_at'] ?? ''),
                    'status' => null,
                    'icon' => 'fa-chart-line',
