...
$pageTitle = 'Cambia stato pratica #' . $practiceId;
...
<div class="flex-grow-1 d-flex flex-column min-vh-100">
    ...
    <main class="content-wrapper">
        ...
        <div class="page-toolbar mb-4 d-flex justify-content-between align-items-start flex-wrap gap-3">
            <div>
                <h1 class="h3 mb-1"><span id="practice-status-title"><?php echo sanitize_output($practiceTitle); ?></span></h1>
                <p class="text-muted mb-0">Aggiorna lo stato della pratica e carica il documento elaborato.</p>
            </div>
        ...
        <div class="card ag-card mb-4">
            <div class="card-body d-flex flex-wrap align-items-center gap-3">
                <span class="badge <?php echo sanitize_output($statusBadgeClass); ?>" id="practice-status-badge"><?php echo sanitize_output($statusLabel); ?></span>
                <span class="badge <?php echo sanitize_output($categoryBadgeClass); ?>" id="practice-status-category"><?php echo sanitize_output($category); ?></span>
                <span class="text-muted">ID: <span id="practice-status-code">#<?php echo $practiceId; ?></span></span>
                <span class="text-muted">Operatore: <span id="practice-status-operator"><?php echo sanitize_output($assignedLabel); ?></span></span>
                <span class="text-muted">Creata il <span id="practice-status-created"><?php echo $createdAt ? sanitize_output(format_datetime_locale($createdAt)) : 'N/D'; ?></span></span>
                <span class="text-muted">Aggiornata il <span id="practice-status-updated"><?php echo $updatedAt ? sanitize_output(format_datetime_locale($updatedAt)) : 'N/D'; ?></span></span>
            </div>
        </div>

        <div id="caf-patronato-practice-status" data-practice-id="<?php echo $practiceId; ?>">
