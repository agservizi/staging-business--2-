...
$pageTitle = 'Modifica ' . ($record['codice'] ?? 'consulenza');
...
$data = [
    'cliente_id' => $record['cliente_id'] !== null ? (string) $record['cliente_id'] : '',
    'intestatario_nome' => $record['intestatario_nome'] ?? '',
    'codice_fiscale' => $record['codice_fiscale'] ?? '',
    'tipo_modello' => $record['tipo_modello'] ?? 'F24',
    'anno_riferimento' => $record['anno_riferimento'] ?? date('Y'),
    'periodo_riferimento' => $record['periodo_riferimento'] ?? '',
    'importo_totale' => (string) ($record['importo_totale'] ?? ''),
    'numero_rate' => (int) ($record['numero_rate'] ?? 1),
    'frequenza_rate' => $record['frequenza_rate'] ?? 'unica',
    'prima_scadenza' => $record['prima_scadenza'] ?? date('Y-m-d'),
    'stato' => $record['stato'] ?? 'bozza',
    'promemoria_scadenza' => $record['promemoria_scadenza'] ?? '',
    'note' => $record['note'] ?? '',
];
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $data['prima_scadenza'] = isset($_POST['prima_scadenza']) ? (string) $_POST['prima_scadenza'] : $data['prima_scadenza'];
    $data['stato'] = in_array($_POST['stato'] ?? '', array_keys($statusOptions), true) ? (string) $_POST['stato'] : $data['stato'];
    ...
    $regenerateSchedule = isset($_POST['rigenera_rate']);
    ...
    if (!$errors) {
        ...
        try {
            ...
            $service->update($consulenzaId, $payload, $userId, $regenerateSchedule);

            if (!empty($_FILES['documento_firmato']['name'])) {
                $service->addDocument($consulenzaId, $_FILES['documento_firmato'], $userId, true);
            }

            add_flash('success', 'Consulenza aggiornata correttamente.');
            header('Location: view.php?id=' . $consulenzaId);
            exit;
        } catch (Throwable $exception) {
            $errors['general'] = 'Impossibile aggiornare la consulenza. Riprova.';
            error_log('Consulenza Fiscale update error: ' . $exception->getMessage());
        }
    }
}
...
<div class="flex-grow-1 d-flex flex-column min-vh-100">
    ...
    <main class="content-wrapper">
        <div class="page-toolbar mb-4 d-flex justify-content-between flex-wrap gap-3">
            <div>
                <h1 class="h3 mb-1">Modifica consulenza <?php echo sanitize_output($record['codice'] ?? ''); ?></h1>
                <p class="text-muted mb-0">Aggiorna i dati fiscali, le scadenze e allega nuovi documenti firmati.</p>
            </div>
            <div class="d-flex gap-2">
                <a class="btn btn-outline-secondary" href="view.php?id=<?php echo $consulenzaId; ?>"><i class="fa-solid fa-eye me-2"></i>Vedi dettaglio</a>
        ...
        <form class="card ag-card" method="post" enctype="multipart/form-data" novalidate>
            ...
            <div class="card-body row g-4">
                ...
                <div class="col-md-3">
                    ...
                    <input class="form-control" id="periodo_riferimento" name="periodo_riferimento" value="<?php echo sanitize_output($data['periodo_riferimento']); ?>">
                </div>
                <div class="col-md-3">
                    <label class="form-label" for="importo_totale">Importo totale *</label>
                    <input class="form-control" id="importo_totale" name="importo_totale" value="<?php echo sanitize_output($data['importo_totale']); ?>" required>
                ...
                <div class="col-md-6">
                    <label class="form-label" for="documento_firmato">Aggiungi nuovo documento firmato</label>
                    <input class="form-control" id="documento_firmato" name="documento_firmato" type="file" accept=".pdf,.jpg,.jpeg,.png">
                    <div class="form-text">PDF, JPG o PNG - massimo 15 MB.</div>
                </div>
                <div class="col-md-6">
                    <label class="form-label" for="rigenera_rate">Rateizzazioni</label>
                    <div class="form-check form-switch">
                        <input class="form-check-input" id="rigenera_rate" name="rigenera_rate" type="checkbox">
                        <label class="form-check-label" for="rigenera_rate">Rigenera automaticamente il piano rate (sovrascrive i dati correnti).</label>
                    </div>
                </div>
            </div>
            <div class="card-footer bg-transparent border-0 d-flex justify-content-end gap-2">
                <a class="btn btn-outline-secondary" href="view.php?id=<?php echo $consulenzaId; ?>">Annulla</a>
